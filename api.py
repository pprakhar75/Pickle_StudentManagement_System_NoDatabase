...
from controller import add_student_to_db, show_student_details, delete_student_from_file, \
...
def add():
    """
    this function takes the post request for addition of the student in file
...
def delete(roll_number):
    """
    This function takes the roll number as the request parameter and delete the  student from the file
    :param roll_number:
    :return: redirect to index
    """
    if roll_number:
        delete_student_from_file(roll_number)
        return redirect(url_for('index'))
